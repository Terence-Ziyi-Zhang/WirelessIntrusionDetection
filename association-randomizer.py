...
import sys, os.path, binascii, signal, subprocess
...
def tshark_cap():
    ...
    subprocess.Popen("tshark -s0 -I -i en0 -f 'not type data' -w /tmp/capture_chan%s.pcap -F pcap" % chan_num,
    ...
    if finished == 'y' or finished == 'yes':
        ...
        subprocess.Popen("mergecap -F pcap -w /tmp/capture_allpcaps.pcap /tmp/capture_*", shell=True,
...
def associations(frame):
    ...
    if frame.type == 0 and frame.subtype == 0:
        ...
        print "Original info:\n Client: %s\n AP: %s\n BSSID: %s\n SSID: %s" % (
            frame.addr2, frame.addr1, frame.addr3, frame.info)
...
def merge_caps():
    subprocess.Popen("mergecap -F pcap -w ./allpcaps.pcap ./assoc_randomized_*", shell=True,
                     stdout=subprocess.PIPE).stdout.read()
...
if len(sys.argv) < 2:
    ...
    sniff(offline=in_file, prn=associations)
